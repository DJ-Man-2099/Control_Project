...
% Last Modified by GUIDE v2.5 08-Jun-2020 18:28:16
...
% Write the input(Ap,Bp) to .h file //////////////////////////////////////
...
function File_Gen(handles)%solve
% hObject    handle to pushbutton2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Ts Bp Ap Hr Hs dist P Bm Am]=acquire_data(handles);
...
%Write input(Ap,Bp,P)to .h file/////////////////
fileID = fopen('Model.h','w');
fprintf(fileID,'#ifndef _MODEL_H_\n');
fclose(fileID);
fileID = fopen('Model.h','a');
fprintf(fileID,'#define _MODEL_H_\n');
fclose(fileID);
fileID = fopen('Model.h','a');
...
fprintf(fileID,'#endif');
fclose(fileID);
%Generate the .c Files for storing the values
%first, the RST .c file
fileID = fopen('RST.c','w');
fprintf(fileID, '#include\"RST.h\"\n');
fprintf(fileID, 'const float R[] = ');
fprintf(fileID, ['{', num2str(R(1))]);
for k=2:numel(R)
fprintf(fileID, [',', num2str(R(k))]);
end
fprintf(fileID, '};\n');
fprintf(fileID, 'const float S[] = ');
fprintf(fileID, ['{', num2str(S(1))]);
for k=2:numel(S)
fprintf(fileID, [',',num2str(S(k))]);
end
fprintf(fileID, '};\n');
fprintf(fileID, 'const float T[] = ');
fprintf(fileID, ['{',num2str(T(1))]);
for k=2:numel(T)
fprintf(fileID, [',', num2str(T(k))]);
end
fprintf(fileID, '};\n');
%Next, the Model file/////////////////
fileID = fopen('Model.c','w');
fprintf(fileID, '#include\"Model.h\"\n');
fprintf(fileID, 'const float Ap[] = ');
fprintf(fileID, ['{', num2str(Ap(1))] );
for k=2:numel(Ap)
fprintf(fileID, [ ',', num2str(Ap(k))]);
end
fprintf(fileID, '};\n');
fprintf(fileID, 'const float Bp[] = ');
fprintf(fileID, ['{', num2str(Bp(1))]);
for k=2:numel(Bp)
fprintf(fileID, [',', num2str(Bp(k))]);
end
fprintf(fileID, '};\n');
fprintf(fileID, 'const float P[] = ');
fprintf(fileID, ['{',num2str(P(1))]);
for k=2:numel(P)
fprintf(fileID, [',', num2str(P(k))]);
end
fprintf(fileID, '};\n');
simulate_RST(Ts, Bp, Ap, R, S, T, Bm, Am, dist, handles);
...
add_block('simulink/Discrete/Discrete Filter', [name '/Bm(z)//Am(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Bm) ']'], 'denominator',['[' num2str(Am) ']'], 'sampletime',str_Ts); %need that
x=x+195;%tracking width
add_line(name, 'Reference/1', 'Bm(z)//Am(z)/1'); %the connector, change that

x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/T(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(T) ']'], 'denominator','[1]', 'sampletime',str_Ts);%need that
x=x+195;%T width
add_line(name, 'Bm(z)//Am(z)/1', 'T(z)/1');% and that

x=x+25;%spacing
add_block('simulink/Math Operations/Sum', [name '/Sum1'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','|+-');%need that
x=x+20;%sum width
add_line(name, 'T(z)/1', 'Sum1/1'); %need that

x=x+20;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/1//Hs(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(Hs) ']'], 'sampletime',str_Ts); %need that
x=x+195;%Hs width
add_line(name, 'Sum1/1', '1//Hs(z)/1'); %and that

x=x+35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/1//S(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(S) ']'], 'sampletime',str_Ts);%need that
x=x+195;%S width
add_line(name, '1//Hs(z)/1', '1//S(z)/1');% and that
...
if get(handles.checkbox1, 'value')%continuous plant
    ...
    x=x+195;%plant width
    add_line(name, '1//S(z)/1', 'Bp(z)//Ap(z)/1');% change that
...
add_line(name, '1//S(z)/1', 'Mux/1');%change that
...
add_block('simulink/Discrete/Discrete Filter', [name '/R(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(R) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');%need that
%all=get(get_param(block_R, 'handle'))
x=x-195;%R width
add_line(name, 'Sum2/1', 'R(z)/1');%change that

x=x-35;%spacing
add_block('simulink/Discrete/Discrete Filter', [name '/Hr(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Hr) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left');%need that
x=x-195;
add_line(name, 'R(z)/1', 'Hr(z)/1');%need that
add_line(name, 'Hr(z)/1)', 'Sum1/2');%need that
%add_line(name, 'R(z)/1)', 'Sum1/2');
open_system(sys);
%clear;

function Code_Gen(handles)%generate Simulink model
% hObject    handle to pushbutton3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Ts Bp Ap Hr Hs dist P Bm Am]=acquire_data(handles);
...
x=x+35;%spacing
blocks=[blocks, add_block('simulink/Discrete/Discrete Filter', [name '/Bm(z)//Am(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Bm) ']'], 'denominator',['[' num2str(Am) ']'], 'sampletime',str_Ts)]; %need that
x=x+195;%tracking width
add_line(name, 'Reference/1', 'Bm(z)//Am(z)/1'); %the connector, change that

x=x+35;%spacing
blocks=[blocks,add_block('simulink/Discrete/Discrete Filter', [name '/T(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(T) ']'], 'denominator','[1]', 'sampletime',str_Ts)];%need that
x=x+195;%T width
add_line(name, 'Bm(z)//Am(z)/1', 'T(z)/1');% and that

x=x+25;%spacing
blocks=[blocks,add_block('simulink/Math Operations/Sum', [name '/Sum1'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','|+-')];%need that
x=x+20;%sum width
add_line(name, 'T(z)/1', 'Sum1/1'); %need that

x=x+20;%spacing
blocks=[blocks,add_block('simulink/Discrete/Discrete Filter', [name '/1//Hs(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(Hs) ']'], 'sampletime',str_Ts)]; %need that
x=x+195;%Hs width
add_line(name, 'Sum1/1', '1//Hs(z)/1'); %and that

x=x+35;%spacing
blocks=[blocks,add_block('simulink/Discrete/Discrete Filter', [name '/1//S(z)'], 'position',[x, y, x+195, y+30],...
    'numerator','[1]', 'denominator',['[' num2str(S) ']'], 'sampletime',str_Ts)];%need that
x=x+195;%S width
add_line(name, '1//Hs(z)/1', '1//S(z)/1');% and that
%add_line(name, 'Sum1/1', '1//S(z)/1');
...
x=x+330;%make space for possible continuous plant
if get(handles.System, 'value')
    blocks=[blocks,add_block('simulink/Math Operations/Sum', [name '/Sum2'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','++|')];
else 
    add_block('simulink/Math Operations/Sum', [name '/Sum2'], 'position',[x, y+5, x+20, y+5+20],...
    'inputs','++|');
...
if get(handles.checkbox1, 'value')%continuous plant
    if get(handles.System, 'value')
        blocks=[blocks, add_block('simulink/Discrete/Zero-Order Hold', [name '/Zero-Order Hold'], 'position',[x, y, x+35, y+30])];
        x=x+35;%ZOH width
        add_line(name, '1//S(z)/1', 'Zero-Order Hold/1');

        x=x+35;%spacing
        blocks=[blocks, add_block('simulink/Continuous/Transport Delay', [name '/Plant Delay'], 'position',[x, y, x+30, y+30],...
            'delaytime',get(handles.edit2, 'string'))];
        x=x+30;%delay width
        add_line(name, 'Zero-Order Hold/1', 'Plant Delay/1');

        x=x+35;%spacing
        blocks=[blocks, add_block('simulink/Continuous/Transfer Fcn', [name '/Gp(s)'], 'position',[x, y, x+120, y+30],...
            'numerator',get(handles.edit3,'string'), 'denominator',get(handles.edit4,'string'))];
        x=x+120;%Gp width
        add_line(name, 'Plant Delay/1', 'Gp(s)/1');
        add_line(name, 'Gp(s)/1', 'Sum2/2');
    ...
    if get(handles.System, 'value')
        blocks=[blocks, add_block('simulink/Discrete/Discrete Filter', [name '/Bp(z)//Ap(z)'], 'position',[x, y, x+195, y+30],...
            'numerator',['[' num2str(Bp) ']'], 'denominator',['[' num2str(Ap) ']'], 'sampletime',str_Ts)];
        x=x+195;%plant width
        add_line(name, '1//S(z)/1', 'Bp(z)//Ap(z)/1');% change that
        add_line(name, 'Bp(z)//Ap(z)/1', 'Sum2/2');
...
%feedback blocks
x=x-465; y=y+110;%from scope left edge to R
blocks=[blocks, add_block('simulink/Discrete/Discrete Filter', [name '/R(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(R) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left')];%need that
%all=get(get_param(block_R, 'handle'))
x=x-195;%R width
add_line(name, 'Sum2/1', 'R(z)/1');%change that
...
x=x-35;%spacing
blocks=[blocks,add_block('simulink/Discrete/Discrete Filter', [name '/Hr(z)'], 'position',[x, y, x+195, y+30],...
    'numerator',['[' num2str(Hr) ']'], 'denominator','[1]', 'sampletime',str_Ts, 'orientation','left')];%need that
x=x-195;
add_line(name, 'R(z)/1', 'Hr(z)/1');%need that
add_line(name, 'Hr(z)/1)', 'Sum1/2');%need that
...
function pushbutton4_Callback(hObject, eventdata, handles) %Added by Us
% hObject    handle to pushbutton4 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
File_Gen(handles)%Solves The Plant
...
% end
% fprintf(fileID2, '}\n');
Code_Gen(handles)%Generates the Simulink Model

% --- Executes on button press in System.
function System_Callback(hObject, eventdata, handles)
% hObject    handle to System (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of System
